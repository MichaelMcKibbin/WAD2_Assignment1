...
export const getMovieCast = (id) => {
  return fetch(
    `XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX${id}/credits?api_key=${process.env.REACT_APP_API_KEY}`
  )
    .then((res) => res.json())
    .then((json) => {
...
export const getMovieVideos = (id) => {
  return fetch(
    `XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX${id}/videos?api_key=${process.env.REACT_APP_API_KEY}`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
...
//   return fetch(
//     `https://api.themoviedb.org/3/person/popular?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&page=2`
//   )
//     .then((response) => {
//       if (!response.ok) {
//         throw new Error(response.json().message);
...
export const getMovieActorDetails = ({ queryKey }) => {
  const [, idPart] = queryKey;
  const { id } = idPart;
  return fetch(
